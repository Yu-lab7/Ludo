...
        if (choice == MENU_ITEM_EXIT) { return; }
    }
}
...
        int current_y = 2;
        attron(A_BOLD);
        mvprintw(start_y + current_y, panel_start_x + 2, "現在のターン: Player %d (%s)", current_player->id, colorToString(current_player->color));
        attroff(A_BOLD);
        current_y += 2;
        
        char dice_text[20];
...
        mvprintw(start_y + current_y, panel_start_x + 2, "サイコロの目: ");
        attron(A_BOLD); printw("%s", dice_text); attroff(A_BOLD);
        current_y += 3;
        
        mvprintw(start_y + current_y++, panel_start_x + 2, "--- Log ---");
        for(int i=0; i<5; i++) { mvprintw(start_y + current_y + i, panel_start_x + 2, ">> %s", state->message_log[i]); }
        
        MenuItem buttons[1];
        int num_buttons = 0;
        if (state->phase == STATE_ROLLING) {
             buttons[num_buttons++] = (MenuItem){"サイコロを振る", board_h - 8, (panel_w - 22)/2, 1, 22, MENU_ITEM_ROLL_DICE};
        }
...
                    int clicked_grid_y = (event.y - start_y) / 2;
                    int clicked_grid_x = (event.x - start_x) / 4;
                    for (int i = 0; i < 4; i++) {
                        Piece *p = &current_player->pieces[i];
                        if (!p->is_movable) continue;
                        Point grid_coords = getGridCoords(p, current_player);
                        if (grid_coords.y == clicked_grid_y && grid_coords.x == clicked_grid_x) {
...
                mvprintw(LINES / 2 - 2 + i, (COLS - 30) / 2, "%d位: Player %d (%s)",
...
        {1,1,1,1,1,1, 5,5,5, 2,2,2,2,2,2},{1,1,0,0,1,1, 5,2,5, 2,2,0,0,2,2},
        {1,0,1,1,0,1, 5,2,5, 2,0,2,2,0,2},{1,0,1,1,0,1, 5,2,5, 2,0,2,2,0,2},
        {1,1,0,0,1,1, 5,2,5, 2,2,0,0,2,2},{1,1,1,1,1,1, 5,2,5, 2,2,2,2,2,2},
        {5,5,5,5,5,5, 6,6,6, 5,5,5,5,5,5},{5,1,1,1,1,1, 6,6,6, 3,3,3,3,3,5},
        {5,5,5,5,5,5, 6,6,6, 5,5,5,5,5,5},{4,4,4,4,4,4, 5,4,5, 3,3,3,3,3,3},
        {4,4,0,0,4,4, 5,4,5, 3,3,0,0,3,3},{4,0,4,4,0,4, 5,4,5, 3,0,3,3,0,3},
        {4,0,4,4,0,4, 5,4,5, 3,0,3,3,0,3},{4,4,0,0,4,4, 5,4,5, 3,3,0,0,3,3},
...
        {{2,2},{2,3},{3,2},{3,3}}, {{2,11},{2,12},{3,11},{3,12}},
        {{11,11},{11,12},{12,11},{12,12}}, {{11,2},{11,3},{12,2},{12,3}}
    };
...
    init_pair(C_YELLOW, COLOR_BLACK, COLOR_YELLOW); init_pair(C_BLUE, COLOR_WHITE, COLOR_BLUE);
...
            } else {
                for (int i = 0; i < num_items; i++) {
                    if (event.y == items[i].y && event.x >= items[i].x && event.x < items[i].x + items[i].width) return items[i].action;
                }
            }
        }
    }
