...
void showGameScreen(GameState *state) {
    ...
    while(1) {
        ...
        int current_y = 4;
        attron(A_BOLD);
        mvprintw(start_y+8, panel_start_x + 20, "現在のターン: Player %d (%s)", current_player->id, colorToString(current_player->color));
        attroff(A_BOLD);
        current_y += 2;
        ...
        if (choice == MENU_ITEM_ROLL_DICE) {
            ...
            if (state->phase == STATE_MOVING_PIECE) {
                MEVENT event;
                int mouse_ok = getmouse(&event);
                
                FILE *fp = fopen("debug2.log", "a");
                if (fp) {
                    fprintf(fp, "getmouse return: %d, event: (%d, %d)\n", mouse_ok, event.y, event.x);
                }
                if (mouse_ok == OK) {
                    int clicked_grid_y = (event.y - start_y - 1) / 2;
                    int clicked_grid_x = (event.x - start_x - 1) / 4;
                    if (fp) {
    ...
    if (ch == KEY_MOUSE) {
        ...
        if (getmouse(&event) == OK && (event.bstate & BUTTON1_CLICKED)) {
            ...
            if (in_panel) {
                ...
                FILE *fp = fopen("debug.log", "a");
                if (fp) {
                    fprintf(fp, "Mouse clicked at (%d, %d)\n", event.y, event.x);
                    fclose(fp);
                }
                return BOARD_CLICK;
            } else {
